...
for _, row in filtered_pair_df.iterrows():
    ...
    for date in zscores.index:
        ...
        # Exit logic
            ...
            if abs(z) < 0.3 or abs(z) > 2.5 or reserved_cash < daily_short_interest :
                # Close position
                if position == 1 :
...
account_value_df.to_excel('Account Values (Overlapped).xlsx')
#print(account_value_df)
# Plotting
import matplotlib.pyplot as plt

account_value_df.plot(figsize=(12, 6), title='Total Account Value Over Time (Overlapped)')
plt.xlabel("Date")
