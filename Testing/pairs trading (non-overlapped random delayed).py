...
# Remove overlapping stock pairs (random order)
used_stocks = set()
filtered_rows = []
...
for _, row in pair_df.iterrows():
    ...
    # Keep the pair only if both stocks haven't been used
    if stock_x not in used_stocks and stock_y not in used_stocks:
        used_stocks.update([stock_x, stock_y])
        filtered_rows.append(row)

# Create new DataFrame with filtered, randomly selected pairs
...
for _, row in filtered_pair_df.iterrows():
    ...
    for date in zscores.index:
        ...
        # Exit logic
        if position != 0:
            # Immediate stop-loss exit
            if abs(z) > 2.3 or reserved_cash < daily_short_interest:
                if position == 1:
                    cash = residual_cash - price_x * units_x
                    cash += units_y * price_y + units_x * price_x - commission * (units_y - units_x)
                elif position == -1:
                    cash = residual_cash - price_y * units_y
                    cash += units_y * price_y + units_x * price_x - commission * (units_x - units_y)
                units_x = units_y = 0
                position = 0
                trade += 1
                exit_pending = False
            ...
            # Start or continue delayed exit
            elif not exit_pending and abs(z) < 0.3:
                if exit_delay_days == 0:
                    # Exit immediately just like the z > 2.3 case
                    if position == 1:
                        cash = residual_cash - price_x * units_x
                        cash += units_y * price_y + units_x * price_x - commission * (units_y - units_x)
                    elif position == -1:
                    ...
                    exit_pending = True
                    exit_countdown = exit_delay_days
...
account_value_df.to_excel('Account Values (Non-Overlapped Random delayed).xlsx')
#print(account_value_df)
# Plotting
import matplotlib.pyplot as plt

account_value_df.plot(figsize=(12, 6), title='Total Account Value Over Time (Non-Overlapped Random Delayed)')
plt.xlabel("Date")
plt.ylabel("Account Value")
plt.grid(True)
plt.savefig('Total_Account_Value_Plot_Non-overlapped Random Delayed.png')
plt.show()