...
# Filter Beta range
pair_df = pair_df[(pair_df['Beta'] > 0.65) & (pair_df['Beta'] < 1.5)]
...
for _, row in filtered_pair_df.iterrows():
    ...
    for date in zscores.index:
        ...
        # Exit logic
            ...
            if abs(z) < 0.5 or abs(z) > 2.3 or reserved_cash < daily_short_interest :
                # Close position
                if position == 1 :
...
account_value_df.to_excel('Account Values (Non-Overlapped random).xlsx')
#print(account_value_df)
# Plotting
import matplotlib.pyplot as plt

account_value_df.plot(figsize=(12, 6), title='Total Account Value Over Time (Non-Overlapped Random)')
plt.xlabel("Date")
plt.ylabel("Account Value")
plt.grid(True)
plt.savefig('Total_Account_Value_Plot_Non-overlapped Random.png')
plt.show()