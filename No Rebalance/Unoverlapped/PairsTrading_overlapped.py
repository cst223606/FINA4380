...
pair_df = pair_df.sort_values(by='beta', ascending=False).reset_index(drop=True)

used_stocks = set()
filtered_pairs = []

for _, row in pair_df.iterrows():  # Use pair_df here
    stock_y = row['stock Y']
    stock_x = row['stock X']

    if stock_y not in used_stocks and stock_x not in used_stocks:
        filtered_pairs.append(row)
        used_stocks.update([stock_y, stock_x])

filtered_pair_df = pd.DataFrame(filtered_pairs)
